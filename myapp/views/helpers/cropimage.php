<?php 
  class CropimageHelper extends Helper {
    ...
    function createForm($imagePath, $tH, $tW){
        ...
        $imgTum = '<img src="'. UPLOADS_PATH . 'av_' . $imagePath . '" alt="" id="thumbnail" />';  
        return $this->output("$imgTum<br style=\"clear:both;\"/>$x1 $y1 $x2 $y2 $w $h $imgP");
        
    } 
    
    function avatarPreview($imagePath, $tH, $tW){
      $imgTumPrev = '<img src="'. UPLOADS_PATH . 'av_' . $imagePath . '" alt="" id="thumbnail-preview" style="position:relative"/>';       
      return "<div style=\"position:relative; overflow:hidden; width:".$tW."px; height:".$tH."px;\">$imgTumPrev</div>";
    }

    function getImagePath($model_name, $path){
